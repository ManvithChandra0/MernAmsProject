...
const CheckOut = () => {
  ...
  useEffect(() => {
    const getUserAddress = async () => {
      await axios.post('https://mernamsproject.onrender.com/getUserAddress', { jwtToken: cookies.get('token') })
        .then((response) => {
      ...
      await axios.post('https://mernamsproject.onrender.com/getCartInfo', { jwtToken: cookies.get('token') })
        .then((response) => {
  ...
  const saveNewAddress = () => {
    if (deliveryData.name.length === 0) {
      ...
      const addData = async () => {
        await axios.post('https://mernamsproject.onrender.com/addAddress', { deliveryData: deliveryData, userId: userAddress.userId })
          .then((response) => {
            setUserAddress(prevAddress => ({
              ...prevAddress,
              address: response.data.address
    ...
    if (userAddress.address.length === undefined) {
      ...
      if (paymentType === 'cash') {
        const addOrder = async () => {
          await axios.post('https://mernamsproject.onrender.com/placecashorder', { userId: userAddress.userId, addressId: selectedAddress, costDetails: cost, paymentType: paymentType })
            .then((response) => {
        ...
        const addOrder = async () => {
          const response = await axios.post('https://mernamsproject.onrender.com/create-order', { amount: cost.payCost + cost.shippingCost })
          const { data } = response
          const prefills = await axios.post('https://mernamsproject.onrender.com/get-checkout-details', { userId: userAddress.userId, addressId: selectedAddress })
          ...
          const options = {
            ...
            handler: async (response) => {
              await axios.post("https://mernamsproject.onrender.com/payment-verification",
                {
