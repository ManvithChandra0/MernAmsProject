...
const NavAccess = (props) => {
  ...
  const [amsCartCollections, setamsCartCollections] = useState([]) /* Entire Items in this Website */
  ...
  useEffect(() => {
    const getAll = async () => {
      await axios.post("https://mernamsproject.onrender.com/getAll", { apparel: props.apparel })
        .then((response) => {
          setamsCartCollections(response.data)
        }).catch(err => {
          console.log(err);
        });

      await axios.post("https://mernamsproject.onrender.com/getWishlistItems", { jwtToken: cookies.get('token') })
        .then((response) => {
  ...
  const wishlistItem = (e, itemId) => {
    if (userWishlist.userId === null && cookie === undefined) {
      ...
      console.log(amsCartCollections[0].defSize)
