...
const NavAccess = (props) => {
  ...
  useEffect(() => {
    const getAll = async () => {
      await axios.post("https://mernamsproject.onrender.com/getAll", { apparel: props.apparel }).then((response) => {
        setCartItems(response.data)
      }).catch(err => {
        console.log(err);
      });

      await axios.post("https://mernamsproject.onrender.com/getUserItems", { jwtToken: cookies.get('token') }).then((response) => {
      ...
      await axios.post("https://mernamsproject.onrender.com/getWishlistItems", { jwtToken: cookies.get('token') }).then((response) => {
  ...
  const changeCartFunctionality = (e, itemId, itemIndex) => {
    ...
    if (usercart.userId === null && cookie === undefined) {
      ...
      //cart added
      axios.post("https://mernamsproject.onrender.com/ddToCart", { itemIdObject: itemId, userIdObject: usercart.userId, size: cartItems[itemIndex].defSize }).then((response) => {
        setUsercart(prevusercart => ({
          ...prevusercart,
          usercartItems: response.data.usercartItems
        }));
  ...
  const wishlistItem = (e, itemId) => {
    if (userWishlist.userId === null && cookie === undefined) {
      ...
      axios.post("https://mernamsproject.onrender.com/arwishlist", { itemIdObject: itemId, userIdObject: userWishlist.userId, ar: e.target.checked }).then((response) => {
