...
const Wishlist = () => {
  ...
  useEffect(() => {
    const getUserWish = async () => {
      await axios.post("https://mernamsproject.onrender.com/getUserWishlist", { jwtToken: cookies.get('token') })
        .then((response) => {
  ...
  const moveToCart = (itemId, apparel) => {
    // console.log(itemId)
    const addCart = async () => {
      await axios.post("https://mernamsproject.onrender.com/addBasic", { userId: userWishlist.userId, itemId: itemId, apparel: apparel })
        .then((response) => {
          setUserWishlist((prevUserWishlist) => ({
            ...prevUserWishlist,
            userWishlistItems: response.data.userWishlistItems
