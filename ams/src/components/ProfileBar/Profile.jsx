...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const getDetails = async () => {
      ...
      await axios.post("https://mernamsproject.onrender.com/getUserInfo", { jwtToken: cookies.get('token') })
        .then((response) => {
  ...
  const updateData = () => {
    ...
    if (edit.name.length < 3) {
      ...
      const update = async () => {
        await axios.post('https://mernamsproject.onrender.com/updateUserDetails', edit)
          .then((response) => {
            setUserDetails(response.data);
            setEdit(response.data);
