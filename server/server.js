...
const TotalAmsCollections = require('./models/TotalAmsCollections');
...
const transporter = nodemailer.createTransport
  ({
    service: 'gmail',
    auth: {
      user: 'manvithmacbook@gmail.com',
      pass: 'whbchdlqmqpfjihs',
    },
  });
  ...
  try {
    ...
    await User.findOne({ email: userData.email }).exec()
      .then((result) => {
        ...
        if (foundData && foundData.isVerified)
    ...
    const mailOptions =
    {
      from: 'manvithmacbook@gmail.com',
      to: userData.email,
      subject: 'Verify AMS Account',
...
app.post("/getAll", async (req, res) => {
  try {
    const collections = await TotalAmsCollections.find({ apparel: req.body.apparel });
  ...
  if (jwtToken == null)
    ...
    jwt.verify(jwtToken, SECRET_KEY, async (err, decoded) => {
      if (err) {
        ...
        const collections = await TotalAmsCollections.find({ _id: { $in: cartInfo.usercartItems.map((element) => element.itemId) } })
...
app.post("/specificProduct", async (req, res) => {
  const { id } = req.body;
  try {
    const result = await TotalAmsCollections.findById(id);
  ...
  if (jwtToken == null)
    ...
    jwt.verify(jwtToken, SECRET_KEY, async (err, decoded) => {
      if (err) {
        ...
        const totalItems = await TotalAmsCollections.find({})
...
app.post("/addBasic", async (req, res) => {
  ...
  const coll = await UserCart.findOne({ userId: userId });
  const totalColl = await TotalAmsCollections.find({});
...
app.post('/placecashorder', async (req, res) => {
  ...
  const sendColl = await TotalAmsCollections.find({ _id: { $in: cartItems.usercartItems.map((element) => element.itemId) } })
